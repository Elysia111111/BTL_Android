...
//        }
    ...
    private void showDiscoutVoucher(String username){
        ...
        if(listVoucher.isEmpty()){
            Toast.makeText(getContext(),"Bạn chưa có voucher giảm giá hoặc voucher của bạn hong thể áp dụng",Toast.LENGTH_LONG).show();
            return;
        }
        AlertDialog.Builder builder = new AlertDialog.Builder(getContext());
        ...
        builder.setMultiChoiceItems(CodeVoucher, checkedItems, new DialogInterface.OnMultiChoiceClickListener() {
            ...
            public void onClick(DialogInterface dialogInterface, int which, boolean isChecked) {
                ...
                if (isChecked) {
                    ...
                    else applyDiscount = (float) (totalCart - (totalCart * voucher.getDiscount_percent() / 100));
                    ...
                    else applyDiscount = (float) (applyDiscount + (totalCart * voucher.getDiscount_percent() / 100));
                    updateTotal(applyDiscount);
                }
            }
        });
